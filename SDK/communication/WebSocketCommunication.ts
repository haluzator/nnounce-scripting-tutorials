...
export class WebSocketCommunication {
	...
	private eventResultHandlers: Map<string /* requestId */, Consumer<INnounceClientResultEvent>> = new Map<string, Consumer<INnounceClientResultEvent>>();
	...
	private connect() {
		...
		try {
			...
			ws.addEventListener("error", (e: Event) => {
				this.loggerConfig.isEnabledInternal() && logger.warn("{} - trying to reconnect.", e instanceof ErrorEvent ? e.message : "WebSocket error");
				this.reconnect();
			});
			this.socket = ws;
		} catch (e) {
	...
	private sendMessageToSocket(jsonMessage: string, isSubscriptionEvent: boolean) {
		...
		if (isSubscriptionEvent) { // e.g. subscription events are sent on websocket connect
			this.loggerConfig.isEnabledInternal() && logger.warn("Websocket is not connected. Subscription event will be sent once websocket connection is established. Event: {}", jsonMessage);
			return;
		}
		this.disconnectEventBuffer.push(jsonMessage);
		this.loggerConfig.isEnabledInternal() && logger.warn("Websocket is not connected. Event added into disconnected buffer. Disconnected buffer size:{}. Event: {}", this.disconnectEventBuffer.length, jsonMessage);
		return;
