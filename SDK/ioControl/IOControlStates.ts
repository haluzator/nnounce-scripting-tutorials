...
export class IOControlStates {
	...
	constructor(webSocket: WebSocketCommunication, loggerConfig: NnLoggerConfig) {
		...
		this.webSocket.addEventHandler("ioPinStateSubscriptionNotify", (event) => this.onControlStateEvent((event as IoPinStateSubscriptionNotify).states))
	}
	...
	private onControlStateEvent(states: Array<IoControl>) {
		...
		inputStates.forEach(ioControl => {
			...
			if (isChange) {
				this.loggerConfig.isEnabledInternal() && logger.debug("Control input '{}' changed raw value from '{}' to raw value '{}'", ioControl.pin, lastState, ioControl.value);
